<template>
  <div>
    ...
    <div class="mt-4 flex items-center justify-between space-x-4 bot-page-px">
      <div class="flex-auto">
        <div
          class="relative h-9 w-full md:w-1/2 flex-shrink-0"
          searchable="true"
        >
          <svg
          ...
          >
            <svg
              ...
              viewBox="0 0 24 24"
              stroke="currentColor"
              class="w-7 inline-block absolute mt-px right-1 text-gray-400"
              role="presentation"
              style="top: 6px"
            >
              <path
    ...
    <div class="mt-8 bot-page-px-list">
      <v-list-stacked-container>
            ...
            <div class="flex justify-end">
              <div class="ml-3 relative">
                <v-dropdown
                  align="right"
                  :width="72"
                  contentClasses="bot-bg-and-text border bot-border"
                  :closeOnClick="false"
                >
                  <template #trigger>
                    <button>
        ...
        <div v-else>
          <ul class="bot-divide-color divide-y">
            <v-meeet-list-item
              v-for="(meet, key) in meets.data"
              :key="key"
              :meet="meet"
            />
          </ul>
...
const {
  ...
  () =>
    `/api/v1/meets?name=${refUserSearchString.value}&is_sanctioned=${refSanctionedFilter.value}`
);

const getMeets = () => {
  refUserSearchString.value = userSearchString.value;
  refSanctionedFilter.value =
    sanctionedFilter.value === "All"
      ? ""
      : sanctionedFilter.value === "Sanctioned"
      ? true
      : false;
  refresh();
};
